...
/// <summary>Contains the state information about the current status of a Reactive Object.</summary>
/// <typeparam name="TSender">The type of the sender of the property changes.</typeparam>
public interface IExtensionState<out TSender>
    where TSender : IReactiveObject
{
    /// <summary>Gets an observable for when a property is changing.</summary>
    IObservable<IReactivePropertyChangedEventArgs<TSender>> Changing { get; }

    /// <summary>Gets an observable for when the property has changed.</summary>
    IObservable<IReactivePropertyChangedEventArgs<TSender>> Changed { get; }

    /// <summary>Gets a observable for when an exception is thrown.</summary>
    ...
    /// <summary>Raises a property changing event.</summary>
    ...
    /// <summary>Indicates if we are currently sending change notifications.</summary>
    /// <returns>If change notifications are being sent.</returns>
    bool NotificationsEnabled();

    /// <summary>Suppress change notifications until the return value is disposed.</summary>
    /// <returns>A IDisposable which when disposed will re-enable change notifications.</returns>
    IDisposable Suppress();

    /// <summary>Are change notifications currently delayed. Used for Observables change notifications only.</summary>
    /// <returns>If the change notifications are delayed.</returns>
    bool AreChangeNotificationsDelayed();

    /// <summary>Delay change notifications until the return value is disposed.</summary>
    /// <returns>A IDisposable which when disposed will re-enable change notifications.</returns>
    IDisposable Delay();
}
