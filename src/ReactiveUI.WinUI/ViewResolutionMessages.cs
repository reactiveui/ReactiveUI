...
internal static class ViewResolutionMessages
{
    ...
    internal const string UnsafeTemplateHook =
        "AutoDataTemplateBindingHookUnsafe hosts each item in a ViewModelViewHostUnsafe, which closes IViewFor<> over the "
        + "view model's run-time type. Use AutoDataTemplateBindingHook, which uses the generated view lookup, to stay ahead-of-time safe.";
}
