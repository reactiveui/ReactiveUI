...
public class DependencyObjectObservableForProperty : ICreatesObservableForProperty
{
    ...
    /// synchronous scheduler (e.g. <see cref="Sequencer.Immediate"/>) so the handler is detached inline.
    /// </summary>
    internal ISequencer Scheduler
    {
        get => field ?? RxSchedulers.MainThreadScheduler;
        set => field = value;
    }
