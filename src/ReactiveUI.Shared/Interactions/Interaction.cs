...
/// The scheduler to use when invoking handlers, which defaults to <c>Sequencer.CurrentThread</c> if <see langword="null"/>.
/// </param>
/// <remarks>
/// <para>
...
/// </code>
/// </example>
[DebuggerDisplay("Interaction")]
public class Interaction<TInput, TOutput>(ISequencer? handlerScheduler = null) : IInteraction<TInput, TOutput>
{
    /// <summary>The ordered list of registered interaction handlers.</summary>
    private readonly List<Func<IInteractionContext<TInput, TOutput>, IObservable<RxVoid>>> _handlers = [];
    ...
    private readonly ISequencer _handlerScheduler = handlerScheduler ?? Sequencer.CurrentThread;

    /// <summary>Cached handler snapshot reused across <see cref="Handle"/> calls; invalidated when handlers change.</summary>
    private Func<IInteractionContext<TInput, TOutput>, IObservable<RxVoid>>[]? _handlersSnapshot;
        ...
        Func<IInteractionContext<TInput, TOutput>, IObservable<TDontCare>> handler)
    {
        ...
        IObservable<RxVoid> ContentHandler(IInteractionContext<TInput, TOutput> context) =>
    ...
    /// </returns>
    protected Func<IInteractionContext<TInput, TOutput>, IObservable<RxVoid>>[] GetHandlers()
    {
        lock (_sync)
        {
    ...
    private ActionDisposable RegisterHandlerCore(Func<IInteractionContext<TInput, TOutput>, IObservable<RxVoid>> contentHandler)
    {
    ...
    /// <summary>Adds a handler delegate to be invoked for interaction contexts.</summary>
    ...
    private void AddHandler(Func<IInteractionContext<TInput, TOutput>, IObservable<RxVoid>> handler)
    {
        lock (_sync)
        {
    ...
    /// <summary>Removes the specified interaction handler from the collection of registered handlers.</summary>
    ...
    private void RemoveHandler(Func<IInteractionContext<TInput, TOutput>, IObservable<RxVoid>> handler)
    {
        lock (_sync)
        {
