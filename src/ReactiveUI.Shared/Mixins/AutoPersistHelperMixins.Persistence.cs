...
/// <summary>Collection AutoPersist members and the persistence metadata/driver engine for <see cref="AutoPersistHelperMixins"/>.</summary>
public static partial class AutoPersistHelperMixins
{
    /// <summary>Provides AutoPersistCollection extension members for generic notify-collection-changed collections.</summary>
    /// <typeparam name="TItem">The item type.</typeparam>
    /// <typeparam name="TCollection">The collection type.</typeparam>
    /// <param name="this">The reactive collection to watch for changes.</param>
    extension<TItem, TCollection>(TCollection @this)
        ...
        where TCollection : INotifyCollectionChanged, IEnumerable<TItem>
    {
            ...
            Action<TItem> onRemove)
        {
            ...
            var changedDisposable =
                ActOnEveryObject(@this.ToReactiveChangeSet<TCollection, TItem>(), onAdd, onRemove);
    ...
    /// <summary>Creates trimming/AOT-friendly persistence metadata for <typeparamref name="T"/>.</summary>
    /// <typeparam name="T">
    ...
    private static void ApplyChange<TItem>(in ReactiveChange<TItem> change, Action<TItem> onAdd, Action<TItem> onRemove)
        where TItem : IReactiveObject
    {
        switch (change.Reason)
        {
            ...
            default:
                {
                    // Any other change reason is intentionally ignored: AutoPersistCollection only tracks
                    // item add/remove/replace/refresh for per-item persistence wiring.
                    break;
                }
        }
    }

    /// <summary>Gets metadata for a runtime type that is not statically known to the trimmer.</summary>
    ...
    /// </remarks>
    [System.Diagnostics.DebuggerDisplay("HasDataContract = {HasDataContract}, Properties = {PersistablePropertyNames.Count}")]
    public sealed class AutoPersistMetadata
    {
        /// <summary>Initializes a new instance of the <see cref="AutoPersistMetadata"/> class.</summary>
        ...
        /// <summary>Gets a value indicating whether the target type is annotated with <c>[DataContract]</c>.</summary>
        public bool HasDataContract { get; }

        /// <summary>Gets the set of property names annotated with <c>[DataMember]</c>.</summary>
        public ISet<string> PersistablePropertyNames { get; }
    }

    /// <summary>Holds precomputed metadata for a closed generic <typeparamref name="T"/>.</summary>
    /// <typeparam name="T">
    ...
    T>
        where T : IReactiveObject
    {
        /// <summary>Gets the computed persistence metadata for <typeparamref name="T"/>.</summary>
        ...
        where T : IReactiveObject
    {
        ...
        private readonly Func<T, IObservable<RxVoid>> _doPersist;
        ...
        private void RunPersist()
        {
            ...
            var subscription = persist.Subscribe(new DelegateObserver<RxVoid>(static _ => { }));
            lock (_gate)
            {
                if (_disposed)
                {
    ...
    /// <summary>Immutable persistence metadata for a given type.</summary>
    private sealed record PersistMetadata
    {
        /// <summary>Initializes a new instance of the <see cref="PersistMetadata"/> class.</summary>
        /// <param name="hasDataContract">Whether the type is annotated with <c>[DataContract]</c>.</param>
        /// <param name="persistablePropertyNames">The set of property names annotated with <c>[DataMember]</c>.</param>
        ...
        /// <summary>Gets a public metadata wrapper for callers.</summary>
        internal AutoPersistMetadata Public { get; }

        /// <summary>Creates persistence metadata for a statically-known or explicitly-preserved type.</summary>
        ...
        /// <summary>Determines whether a property is annotated with <c>[DataMember]</c>.</summary>
