...
#endif
/// <summary>
/// Combines the latest of 4 sources, emitting the selector result once every source
...
internal sealed class WhenAnyChangeSink<T1, T2, T3, T4, TResult>(
    IObservable<T1> source1,
    IObservable<T2> source2,
    IObservable<T3> source3,
    IObservable<T4> source4,
    Func<T1, T2, T3, T4, TResult> selector) : IObservable<TResult>
{
    /// <inheritdoc/>
    public IDisposable Subscribe(IObserver<TResult> observer)
    {
        ...
        sink.Run(source1, source2, source3, source4);
        return sink;
    }
    ...
    /// <param name="selector">Combines the ready values into a result.</param>
    private sealed class Sink(IObserver<TResult> downstream, Func<T1, T2, T3, T4, TResult> selector) : IDisposable
    {
        ...
        private void Emit()
        {
            ...
            try
            {
                result = selector(_value1, _value2, _value3, _value4);
            }
            catch (Exception ex)
            {
