...
/// <summary>
/// Projects a single source through a selector. The fused arity-1 form of the
/// WhenAny combine sink: no gate is needed for one source.
/// </summary>
...
internal sealed class WhenAnyChangeSink<T1, TResult>(
    ...
    Func<T1, TResult> selector) : IObservable<TResult>
{
    ...
    /// <summary>Projects each source notification through the selector and forwards the result.</summary>
    /// <param name="downstream">The observer receiving the projected results.</param>
    /// <param name="selector">Projects the source element into the result.</param>
    private sealed class Sink(IObserver<TResult> downstream, Func<T1, TResult> selector) : IObserver<T1>
    {
        /// <inheritdoc/>
