...
#endif
/// <summary>
/// Combines the latest of 7 sources, emitting the selector result once every source
...
internal sealed class WhenAnyChangeSink<T1, T2, T3, T4, T5, T6, T7, TResult>(
    ...
    Func<T1, T2, T3, T4, T5, T6, T7, TResult> selector) : IObservable<TResult>
{
    /// <inheritdoc/>
    public IDisposable Subscribe(IObserver<TResult> observer)
    {
        ...
        sink.Run(source1, source2, source3, source4, source5, source6, source7);
        return sink;
    }
    ...
    /// <param name="selector">Combines the ready values into a result.</param>
    private sealed class Sink(IObserver<TResult> downstream, Func<T1, T2, T3, T4, T5, T6, T7, TResult> selector) : IDisposable
    {
        ...
        private void On1(T1 change)
        {
            lock (_gate)
            {
                _value1 = change;
                _has1 = true;
                if (!(_has2 && _has3 && _has4 && _has5 && _has6 && _has7))
                {
                    return;
                }
        ...
        private void On2(T2 change)
        {
            lock (_gate)
            {
                _value2 = change;
                _has2 = true;
                if (!(_has1 && _has3 && _has4 && _has5 && _has6 && _has7))
                {
                    return;
                }
        ...
        private void On3(T3 change)
        {
            lock (_gate)
            {
                _value3 = change;
                _has3 = true;
                if (!(_has1 && _has2 && _has4 && _has5 && _has6 && _has7))
                {
                    return;
                }
        ...
        private void On4(T4 change)
        {
            lock (_gate)
            {
                _value4 = change;
                _has4 = true;
                if (!(_has1 && _has2 && _has3 && _has5 && _has6 && _has7))
                {
                    return;
                }
        ...
        private void On5(T5 change)
        {
            lock (_gate)
            {
                _value5 = change;
                _has5 = true;
                if (!(_has1 && _has2 && _has3 && _has4 && _has6 && _has7))
                {
                    return;
                }
        ...
        private void On6(T6 change)
        {
            lock (_gate)
            {
                _value6 = change;
                _has6 = true;
                if (!(_has1 && _has2 && _has3 && _has4 && _has5 && _has7))
                {
                    return;
                }
        ...
        private void Emit()
        {
            ...
            try
            {
                result = selector(_value1, _value2, _value3, _value4, _value5, _value6, _value7);
            }
            catch (Exception ex)
            {
