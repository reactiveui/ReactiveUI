...
#endif
/// <summary>A subject which dispatches all its events on the specified Scheduler.</summary>
/// <typeparam name="T">The type of item being dispatched by the Subject.</typeparam>
[System.Diagnostics.DebuggerDisplay("Observers = {_observerRefCount}")]
public class ScheduledSubject<T> : ISignal<T>
{
    ...
    /// <summary>Whether this subject has been disposed.</summary>
    private bool _isDisposed;

    /// <summary>Initializes a new instance of the <see cref="ScheduledSubject{T}"/> class with only a scheduler.</summary>
    /// <param name="scheduler">The scheduler where to dispatch items to.</param>
    public ScheduledSubject(ISequencer scheduler)
        : this(scheduler, null, null)
    {
    }

    /// <summary>Initializes a new instance of the <see cref="ScheduledSubject{T}"/> class with a scheduler and a default observer.</summary>
    /// <param name="scheduler">The scheduler where to dispatch items to.</param>
    ...
    public ScheduledSubject(ISequencer scheduler, IObserver<T>? defaultObserver)
        : this(scheduler, defaultObserver, null)
    {
    }

    /// <summary>Initializes a new instance of the <see cref="ScheduledSubject{T}"/> class.</summary>
    /// <param name="scheduler">The scheduler where to dispatch items to.</param>
    ...
    /// <summary>Disposes of any managed resources in our class.</summary>
