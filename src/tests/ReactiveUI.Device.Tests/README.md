...
- **Android** (`net11.0-android37`). An instrumentation runs every test in the app. `dotnet test --device` starts it,
  streams each result back and writes a TRX report on the host.
- **iOS** (`net11.0-ios`). When UIKit connects the app's scene, the app runs every test, writes a TRX report and an
  exit-code file, and exits. It builds on Windows and macOS. It runs only on a macOS simulator.
...
```bash
dotnet test --project tests/ReactiveUI.Device.Tests/ReactiveUI.Device.Tests.csproj -f net11.0-android37 --device emulator-5554 --report-trx
```
