...
/// <summary>
/// Helper for switching the active mode detector during tests that require live runtime threads.
/// </summary>
public static class LiveModeDetector
{
    /// <summary>
    /// The detector that always reports not being in a unit test runner.
    /// </summary>
    private static readonly AlwaysFalseModeDetector liveModeDetector = new();

    /// <summary>
    /// The default mode detector.
    /// </summary>
    private static readonly DefaultModeDetector defaultModeDetector = new();

    /// <summary>
    /// Overrides the mode detector so the runtime behaves as if not in a unit test runner.
    /// </summary>
    public static void UseRuntimeThreads() =>
        ModeDetector.OverrideModeDetector(liveModeDetector);

    /// <summary>
    /// Restores the default mode detector.
    /// </summary>
    public static void UseDefaultModeDetector() =>
        ModeDetector.OverrideModeDetector(defaultModeDetector);

    /// <summary>
    /// Gets a value indicating whether the code is running in a unit test runner.
    /// </summary>
