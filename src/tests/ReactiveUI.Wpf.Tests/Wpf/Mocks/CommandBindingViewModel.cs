...
/// <summary>A mock view model used by WPF command binding tests.</summary>
public class CommandBindingViewModel : ReactiveObject
{
    /// <summary>The delay, in milliseconds, applied by the asynchronous command body.</summary>
    private const int CommandDelayMilliseconds = 1000;

    /// <summary>The result value produced by the asynchronous command body.</summary>
    private const int CommandResultValue = 100;

    /// <summary>Backing field for the <see cref="Result"/> property.</summary>
    private readonly ObservableAsPropertyHelper<int?> _result;

    /// <summary>Backing field for the <see cref="Command1"/> property.</summary>
    private ReactiveCommand<int, int> _command1;

    /// <summary>Backing field for the <see cref="Command2"/> property.</summary>
    private ReactiveCommand<RxVoid, RxVoid> _command2;

    /// <summary>Backing field for the <see cref="Command3"/> property.</summary>
    private ReactiveCommand<RxVoid, int?> _command3;
    ...
    {
        _command1 = ReactiveCommand.Create<int, int>(static _ => _, outputScheduler: Sequencer.Immediate);
        _command2 = ReactiveCommand.Create(static () => { }, outputScheduler: Sequencer.Immediate);
        _command3 = ReactiveCommand.CreateFromTask(RunAsync, outputScheduler: RxSchedulers.TaskpoolScheduler);
        _result = _command3.ToProperty(this, static x => x.Result, scheduler: Sequencer.Immediate);
    }
    ...
    public ReactiveCommand<RxVoid, int?> Command3
    {
        get => _command3;
    ...
    /// <summary>Gets the result produced by the third command.</summary>
