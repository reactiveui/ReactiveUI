...
public class SuspensionHostExtensionsTests
{
    ...
    public async Task SetupDefaultSuspendResume_WithNullDriver_LogsError()
    {
        ...
        try
        {
            ...
            var disposable = host.SetupDefaultSuspendResume(null);
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task GetAppStateThrowsForNullHost()
    {
        await Assert.That(() => ((ISuspensionHost)null!).GetAppState<DummyAppState>()).Throws<ArgumentNullException>();
    }
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ObserveAppStateThrowsForNullHost()
    {
