...
/// <summary>A view model used to exercise <see cref="ReactiveProperty{T}"/> validation scenarios.</summary>
public class ReactivePropertyVm : ReactiveObject
{
    /// <summary>The maximum allowed length for <see cref="LengthLessThanFiveProperty"/>.</summary>
    private const int MaxLength = 5;

    /// <summary>Initializes a new instance of the <see cref="ReactivePropertyVm"/> class using the default scheduler.</summary>
    public ReactivePropertyVm()
        : this(null)
    {
    }

    /// <summary>Initializes a new instance of the <see cref="ReactivePropertyVm"/> class.</summary>
    /// <param name="scheduler">The scheduler used by the reactive properties.</param>
    public ReactivePropertyVm(ISequencer? scheduler)
    {
    ...
    /// <summary>Gets a property that validates with a custom error message.</summary>
    ...
    /// <summary>Gets a property that validates with a custom error message using a display name.</summary>
    [Required(ErrorMessage = "Custom validation error message for {0}")]
    [Display(Name = "CustomName")]
    public ReactiveProperty<string> CustomValidationErrorMessageWithDisplayNameProperty { get; }

    /// <summary>Gets a property that validates with a custom error message sourced from a resource.</summary>
    [Required(
    ...
    /// <summary>Gets a property that is required.</summary>
    [Required(ErrorMessage = "error!")]
    public ReactiveProperty<string> IsRequiredProperty { get; }

    /// <summary>Gets a property whose length must be less than five characters.</summary>
    [StringLength(MaxLength, ErrorMessage = "5over")]
    public ReactiveProperty<string> LengthLessThanFiveProperty { get; }

    /// <summary>Gets a property validated by an asynchronous validation task.</summary>
