...
{
    /// <summary>Verifies the arity-11 value sink emits when every source is ready and re-emits per later source update.</summary>
    ...
    public async Task ValueSink11_Emits()
    {
        ...
        using var sub = new WhenAnyValueSink<object?, string, string, string, string, string, string, string, string, string, string, string, string>(
            ...
            s10,
            s11,
            static (x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11) => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11).Subscribe(rec);
        s1.OnNext(Ch("v1"));
        s2.OnNext(Ch("v2"));
        s3.OnNext(Ch("v3"));
        s4.OnNext(Ch("v4"));
    ...
    /// <summary>Verifies the arity-11 value sink forwards a source error to the observer.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ValueSink11_ForwardsError()
    {
        ...
        var rec = new Recorder<string>();
        _ = new WhenAnyValueSink<object?, string, string, string, string, string, string, string, string, string, string, string, string>(
            e1,
            e2,
            e3,
            e4,
    ...
    /// <summary>Verifies the arity-11 value sink completes once every source completes.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    ...
    /// <summary>Verifies a throwing selector surfaces as an error from the arity-11 value sink.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
