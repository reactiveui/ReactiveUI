...
{
    /// <summary>Verifies the arity-10 change sink emits when every source is ready and re-emits per later source update.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ChangeSink10_Emits()
    {
        ...
        using var sub = new WhenAnyChangeSink<string, string, string, string, string, string, string, string, string, string, string>(
            s1,
            s2,
            s3,
            s4,
            ...
            s9,
            s10,
            static (x1, x2, x3, x4, x5, x6, x7, x8, x9, x10) => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10).Subscribe(rec);
        s1.OnNext("v1");
        s2.OnNext("v2");
        s3.OnNext("v3");
        s4.OnNext("v4");
    ...
    /// <summary>Verifies the arity-10 change sink forwards a source error to the observer.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ChangeSink10_ForwardsError()
    {
        ...
        var rec = new Recorder<string>();
        _ = new WhenAnyChangeSink<string, string, string, string, string, string, string, string, string, string, string>(
            e1,
            e2,
            e3,
            e4,
    ...
    /// <summary>Verifies the arity-10 change sink completes once every source completes.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ChangeSink10_Completes()
    {
    ...
    /// <summary>Verifies a throwing selector surfaces as an error from the arity-10 change sink.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ChangeSink10_SelectorThrows()
    {
