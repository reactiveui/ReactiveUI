...
/// <content>Direct arity-8 <c>WhenAnyValueSink</c> coverage.</content>
...
{
    /// <summary>Verifies the arity-8 value sink emits when every source is ready and re-emits per later source update.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ValueSink8_Emits()
    {
        ...
        using var sub = new WhenAnyValueSink<object?, string, string, string, string, string, string, string, string, string>(
            s1,
            s2,
            s3,
            s4,
    ...
    /// <summary>Verifies the arity-8 value sink forwards a source error to the observer.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ValueSink8_ForwardsError()
    {
        ...
        var rec = new Recorder<string>();
        new WhenAnyValueSink<object?, string, string, string, string, string, string, string, string, string>(
            e1,
            e2,
            e3,
            e4,
    ...
    /// <summary>Verifies the arity-8 value sink completes once every source completes.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ValueSink8_Completes()
    {
    ...
    /// <summary>Verifies a throwing selector surfaces as an error from the arity-8 value sink.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task ValueSink8_SelectorThrows()
    {
        ...
        new WhenAnyValueSink<object?, string, string, string, string, string, string, string, string, string>(
            ...
            t7,
            t8,
            (_, _, _, _, _, _, _, _) => throw ex).Subscribe(rec);
        t1.OnNext(Ch("a"));
        t2.OnNext(Ch("a"));
        t3.OnNext(Ch("a"));
        t4.OnNext(Ch("a"));
