...
{
    /// <summary>Verifies the WhenAnyObservable overload for 7 observable properties.</summary>
    ...
    public async Task WhenAnyObservable_7Props()
    {
        ...
        vm.WhenAnyObservable(
            ...
            x => x.ObservableProperty7).ObserveOn(Sequencer.Immediate).Subscribe(list.Add);
        subj1.OnNext("test");
        subj2.OnNext("test");
        subj3.OnNext("test");
        subj4.OnNext("test");
    ...
    /// <summary>Verifies the WhenAnyObservable overload for 7 observable properties with a selector.</summary>
    /// <returns>A <see cref="Task" /> representing the asynchronous operation.</returns>
    [Test]
