...
/// <summary>Detects if we are in production mode or not.</summary>
public class ProductionMode : IModeDetector, IPlatformModeDetector
{
    /// <summary>The shared instance used as the override mode detector.</summary>
    private static readonly ProductionMode Instance = new();

    /// <summary>Sets the platform mode.</summary>
    ...
    /// <summary>Value indicating whether we are in the unit test runner.</summary>
    /// <returns>If we are in test mode.</returns>
    public bool? InUnitTestRunner() => false;

    /// <summary>Value indicating whether we are in the design mode.</summary>
