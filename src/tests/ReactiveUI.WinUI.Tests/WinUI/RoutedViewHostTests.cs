...
public class RoutedViewHostTests
{
    ...
    {
        var host = new RoutedViewHost { DefaultContent = Placeholder };

        using (Assert.Multiple())
        {
            await Assert.That(host.DefaultContent).IsEqualTo(Placeholder);
            await Assert.That(host.GetValue(RoutedViewHost.DefaultContentProperty)).IsEqualTo(Placeholder);
        }
    }
    ...
    public async Task Router_RoundTripsThroughTheDependencyProperty()
    {
        ...
        await Assert.That(host.GetValue(RoutedViewHost.RouterProperty)).IsSameReferenceAs(router);
    }
    ...
    {
        var host = new RoutedViewHost { ViewContract = WideContract };
    ...
    public async Task Navigate_ShowsTheResolvedView()
    {
        ...
        var host = new RoutedViewHost { DefaultContent = Placeholder, ViewLocator = locator, Router = router };
    ...
    /// <summary>Verifies a view model routed to before the router is attached is still resolved.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task Router_AssignedAfterNavigation_ShowsTheResolvedView()
    {
        var view = new RoutedTestView();
        var router = new RoutingState(Sequencer.Immediate);
        using var navigation = router.Navigate.Execute(new RoutedTestViewModel()).Subscribe();
        var host = new RoutedViewHost { ViewLocator = new StubViewLocator { ContractlessView = view } };
    ...
    public async Task ViewContract_SelectsTheContractSpecificView()
    {
        ...
        var host = new RoutedViewHost { ViewLocator = locator, Router = router, ViewContract = WideContract };
    ...
    public async Task ViewContract_WithNoContractSpecificView_FallsBackToTheContractlessView()
    {
        ...
        var host = new RoutedViewHost { ViewLocator = locator, Router = router, ViewContract = unknownContract };
    ...
    {
        var router = new RoutingState(Sequencer.Immediate);
        var host = new RoutedViewHost { ViewLocator = new StubViewLocator(), Router = router };
