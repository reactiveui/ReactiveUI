...
#endif
/// <summary>Control fetcher helpers for Android that support wiring up properties to Android resource IDs by name.</summary>
/// <remarks>
/// <para>
...
public static partial class ControlFetcherMixins
{
    /// <summary>Cache mapping an assembly to a case-insensitive resource-name→id map.</summary>
    /// <remarks>
    ...
    /// <summary>Cache mapping a root view object to a per-property cached <see cref="View"/> instance.</summary>
    /// <remarks>
    /// <para>
    ...
    /// <summary>Cache of wire-up property lists per runtime type and resolve strategy.</summary>
    /// <remarks>
    ...
    /// <summary>The activity that hosts the view hierarchy or whose properties should be wired.</summary>
    /// <param name="activity">The hosting <see cref="Activity"/> whose declared control properties are resolved and assigned.</param>
    extension(Activity activity)
    {
        /// <summary>Gets a control from an <see cref="Activity"/>, resolving the resource name (defaulting to the calling member name).</summary>
        /// <param name="propertyName">The activity resource name to resolve. Defaults to the calling member's name.</param>
        /// <returns>The view hosted by the activity if found; otherwise <see langword="null"/>.</returns>
        ...
        /// <summary>Wires view controls to properties on an <see cref="Activity"/>.</summary>
        /// <param name="resolveMembers">The resolve strategy controlling which activity properties are wired.</param>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="activity"/> is null.</exception>
        /// <exception cref="MissingFieldException">
        /// Thrown when a property cannot be wired to a view with a corresponding resource identifier.
        /// </exception>
        [RequiresUnreferencedCode(
            "WireUpControls uses reflection to discover properties and attributes that may be trimmed.")]
        [RequiresDynamicCode("WireUpControls uses reflection that may require dynamic code generation.")]
        public void WireUpControls(ResolveStrategy resolveMembers)
        {
    ...
    /// <summary>The fragment whose properties should be wired.</summary>
    /// <param name="fragment">The <see cref="Fragment"/> whose declared control properties are resolved against its inflated view.</param>
    extension(Fragment fragment)
    {
        /// <summary>Wires view controls to properties on an Android <see cref="Fragment"/> using the implicit resolve strategy.</summary>
        ...
        /// <summary>Wires view controls to properties on an Android <see cref="Fragment"/>.</summary>
        /// <param name="inflatedView">The inflated view returned from <c>OnCreateView</c>.</param>
        /// <param name="resolveMembers">The resolve strategy controlling which fragment properties are wired.</param>
        /// <exception cref="ArgumentNullException">
        /// Thrown when <paramref name="fragment"/> or <paramref name="inflatedView"/> is null.
        /// </exception>
        /// <exception cref="MissingFieldException">
    ...
    /// <summary>The layout host that exposes a <see cref="ILayoutViewHost.View"/>.</summary>
    /// <param name="layoutHost">The <see cref="ILayoutViewHost"/> whose <see cref="ILayoutViewHost.View"/> is used as the root for resolving control properties.</param>
    extension(ILayoutViewHost layoutHost)
    {
        /// <summary>Wires view controls to properties on an <see cref="ILayoutViewHost"/> using the implicit resolve strategy.</summary>
        ...
        /// <summary>Wires view controls to properties on an <see cref="ILayoutViewHost"/>.</summary>
        /// <param name="resolveMembers">The resolve strategy controlling which layout-host properties are wired.</param>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="layoutHost"/> is null.</exception>
        /// <exception cref="MissingFieldException">
        /// Thrown when a property cannot be wired to a view with a corresponding resource identifier.
        /// </exception>
    ...
    /// <summary>The property being wired.</summary>
    /// <param name="member">The <see cref="PropertyInfo"/> describing the property whose resource name is being resolved.</param>
    extension(PropertyInfo member)
    {
        /// <summary>Gets the resource name for the specified property based on optional overrides.</summary>
    ...
    /// <summary>The view from which to retrieve a control or whose properties should be wired.</summary>
    /// <param name="view">The <see cref="View"/> used as the root for child-control lookups and property wire-up.</param>
    extension(View view)
    {
        /// <summary>Gets a control from an Android <see cref="View"/>, resolving the resource name (defaulting to the calling member name).</summary>
        /// <param name="assembly">The assembly containing the user-defined view and its resources.</param>
        /// <param name="propertyName">The child-control resource name to resolve. Defaults to the calling member's name.</param>
        /// <returns>The child view found beneath this view if present; otherwise <see langword="null"/>.</returns>
        [RequiresUnreferencedCode(
            "Android resource discovery uses reflection over generated resource types that may be trimmed.")]
        [RequiresDynamicCode("Android resource discovery uses reflection that may require dynamic code generation.")]
        [SuppressMessage(
        ...
        /// <summary>Wires view controls to properties on an Android <see cref="View"/>.</summary>
        /// <param name="resolveMembers">The resolve strategy controlling which view properties are wired.</param>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="view"/> is null.</exception>
        /// <exception cref="MissingFieldException">
        /// Thrown when a property cannot be wired to a view with a corresponding resource identifier.
        /// </exception>
    ...
    /// <summary>The object whose properties are to be discovered for wire-up.</summary>
    /// <param name="this">The receiver instance whose runtime type is reflected over to enumerate wire-up candidates.</param>
    extension(object @this)
    {
        ...
        public PropertyInfo[] GetWireUpMembers(ResolveStrategy resolveStrategy)
        {
            ...
            return GetWireUpMembersCached(type, resolveStrategy);
        }
    }

    /// <summary>Returns the set of properties eligible for wiring for a given runtime type and strategy.</summary>
    ...
    /// <summary>Determines whether a property should be wired up for the given resolve strategy.</summary>
    ...
    /// <param name="rootView">The root view object that scopes the per-root cache.</param>
    /// <param name="fetchControlFromView">Stateless factory invoked to resolve the view on a cache miss.</param>
    /// <returns>The cached or newly fetched view (possibly null).</returns>
    ...
    /// <summary>Gets a cached control for a root view and property name, fetching it if absent, with an extra state value.</summary>
    /// <typeparam name="TState">The type of state passed to the fetch function.</typeparam>
    /// <param name="propertyName">The cache key for this stateful lookup, typically the property name.</param>
    /// <param name="rootView">The root view object that scopes the per-root cache for this lookup.</param>
    /// <param name="fetchControlFromView">State-carrying factory invoked to resolve the view on a cache miss.</param>
    /// <param name="state">State passed to the fetch factory.</param>
    /// <returns>The cached or newly fetched view for the stateful lookup (possibly null).</returns>
    ...
    /// <summary>Resolves the Android resource ID for the given resource name within the specified assembly.</summary>
    ...
    /// <summary>Locates the generated Android resource type for the specified assembly.</summary>
    /// <param name="assembly">The assembly whose referenced designer or modules are searched for the resource type.</param>
...
#if NET8_0_OR_GREATER
    /// <summary>Finds the name of the referenced Android resource designer assembly, if present.</summary>
    ...
    /// <summary>Searches the supplied modules for a type with the given name.</summary>
