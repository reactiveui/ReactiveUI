...
namespace ReactiveUI
{
    ...
    /// <remarks>
    /// <para>
    /// Interactions allow collaborating components to asynchronously ask questions of each other.
    /// Typically, a view model wants to ask the user a question before proceeding with some
    /// operation, and it's the view that provides the interface via which users can answer the question.
    /// </para>
    /// <para>
    /// Interactions have both an input and output, both of which are strongly-typed via generic type
    /// parameters. The input is passed into the interaction so that handlers have the information
    /// they require. The output is provided by a handler.
    /// </para>
    /// <para>
    /// By default, handlers are invoked in reverse order of registration. That is, handlers
    /// registered later are given the opportunity to handle interactions before handlers that were
    /// registered earlier. This chaining mechanism enables handlers to be registered temporarily in
    /// a specific context, such that interactions can be handled in a different manner. Subclasses
    /// may modify this behavior by overriding the <see cref="Handle"/> method.
    /// </para>
    /// <para>
    /// Note that handlers are not required to handle an interaction. They can choose to ignore it,
    /// leaving it for some other handler to handle. If no handler handles the interaction, the <see
    /// cref="Handle"/> method will throw an UnhandledInteractionException.
    /// </para>
    /// </remarks>
    ...
    public class Interaction<TInput, TOutput>
    {
        ...
        /// The scheduler to use when invoking handlers, which defaults to
        /// <c>CurrentThreadScheduler.Instance</c> if <see langword="null"/>.
        /// </param>
        ...
        /// <remarks>
        /// <para>
        /// This method passes the interaction through to relevant handlers in reverse order of
        /// registration, ceasing once any handler handles the interaction. If the interaction
        /// remains unhandled after all relevant handlers have executed, an
        /// UnhandledInteractionException is thrown.
        /// </para>
        /// </remarks>
        /// <param name="input">The input for the interaction.</param>
        /// <returns>
        /// An observable that ticks when the interaction completes, or throws an
        /// UnhandledInteractionException if no handler handles the interaction.
        /// </returns>
        ...
        /// <remarks>
        /// <para>
        /// This overload of <c>RegisterHandler</c> is only useful if the handler can handle the
        /// interaction immediately. That is, it does not need to wait for a user or some other
        ...
        /// <remarks>
        /// <para>
        /// This overload of <c>RegisterHandler</c> is useful if the handler needs to perform some
    ...
    /// <remarks>
    /// <para>
    /// Instances of this class are passed into interaction handlers. The <see cref="Input"/>
    /// property exposes the input to the interaction, whilst the <see cref="SetOutput"/> method
    /// allows a handler to provide the output.
    /// </para>
    /// </remarks>
    ...
    public sealed class InteractionContext<TInput, TOutput>
    {
        ...
        /// <summary>
        /// Gets a value indicating whether the interaction is handled. That is, whether the output
        /// has been set.
        /// </summary>
        ...
        /// <exception cref="InvalidOperationException">If the output has already been set.</exception>
    ...
    public class UnhandledInteractionException<TInput, TOutput> : Exception
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UnhandledInteractionException{TInput,
        /// TOutput}"/> class.
        /// </summary>
