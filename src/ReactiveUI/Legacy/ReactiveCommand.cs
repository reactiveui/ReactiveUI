...
{
    /// <summary>
    /// interface for ReactiveCommand
    /// </summary>
    /// <seealso cref="ReactiveUI.IHandleObservableErrors"/>
    ...
    public interface IReactiveCommand : IHandleObservableErrors, ICommand, IDisposable, IEnableLogger
    {
        ...
        /// <summary>
        /// Gets a value indicating whether this instance is executing. This Observable is guaranteed
        /// to always return a value immediately (i.e. it is backed by a BehaviorSubject), meaning it
        /// is safe to determine the current state of the command via IsExecuting.First()
        /// </summary>
    ...
    /// <summary>
    /// IReactiveCommand represents an ICommand which also notifies when it is executed (i.e. when
    /// Execute is called) via IObservable. Conceptually, this represents an Event, so as a result
    /// this IObservable should never OnComplete or OnError. /// In previous versions of ReactiveUI,
    /// this interface was split into two separate interfaces, one to handle async methods and one
    /// for "standard" commands, but these have now been merged - every ReactiveCommand is now a ReactiveAsyncCommand.
    /// </summary>
    [Obsolete("This type is obsolete and will be removed in a future version of ReactiveUI. Please switch to using ReactiveUI.ReactiveCommand instead.")]
    ...
    {
        /// <summary>
        /// Creates a default ReactiveCommand that has no background action. This is probably what
        /// you want if you were calling the constructor in previous versions of ReactiveUI
        /// </summary>
        /// <param name="canExecute">
        /// An Observable that determines when the Command can Execute. WhenAny is a great way to
        /// create this!
        /// </param>
        /// <param name="scheduler">The scheduler to deliver events on. Defaults to RxApp.MainThreadScheduler.</param>
        /// <returns>
        /// A ReactiveCommand whose ExecuteAsync just returns the CommandParameter immediately. Which
        /// you should ignore!
        /// </returns>
        ...
        /// <summary>
        /// Creates a ReactiveCommand typed to the given executeAsync Observable method. Use this
        /// method if your background method returns IObservable.
        /// </summary>
        /// <param name="canExecute">
        /// An Observable that determines when the Command can Execute. WhenAny is a great way to
        /// create this!
        ...
        /// Method to call that creates an Observable representing an operation to execute in the
        /// background. The Command's CanExecute will be false until this Observable completes. If
        /// this Observable terminates with OnError, the Exception is marshaled to ThrownExceptions.
        /// </param>
        /// <param name="scheduler">The scheduler to deliver events on. Defaults to RxApp.MainThreadScheduler.</param>
        /// <returns>
        /// A ReactiveCommand which returns all items that are created via calling executeAsync as a
        /// single stream.
        /// </returns>
        ...
        /// <summary>
        /// Creates a ReactiveCommand typed to the given executeAsync Task-based method. Use this
        /// method if your background method returns Task or uses async/await.
        /// </summary>
        /// <param name="canExecute">
        /// An Observable that determines when the Command can Execute. WhenAny is a great way to
        /// create this!
        /// </param>
        /// <param name="executeAsync">
        /// Method to call that creates a Task representing an operation to execute in the
        /// background. The Command's CanExecute will be false until this Task completes. If this
        /// Task terminates with an Exception, the Exception is marshaled to ThrownExceptions.
        /// </param>
        /// <param name="scheduler">The scheduler to deliver events on. Defaults to RxApp.MainThreadScheduler.</param>
        /// <returns>
        /// A ReactiveCommand which returns all items that are created via calling executeAsync as a
        ...
        /// <summary>
        /// Creates a ReactiveCommand typed to the given executeAsync Task-based method that supports
        /// cancellation. Use this method if your background method returns Task or uses async/await.
        /// </summary>
        /// <param name="canExecute">
        /// An Observable that determines when the Command can Execute. WhenAny is a great way to
        /// create this!
        ...
        /// <summary>
        /// This creates a ReactiveCommand that calls several child ReactiveCommands when invoked.
        /// Its CanExecute will match the combined result of the child CanExecutes (i.e. if any child
        /// commands cannot execute, neither can the parent)
        /// </summary>
        /// <param name="canExecute">
        /// An Observable that determines whether the parent command can execute
        /// </param>
    ...
    /// <summary>
    /// This class represents a Command that can optionally do a background task. The results of the
    /// background task (or a signal that the Command has been
    /// invoked) are delivered by Subscribing to the command itself, since ReactiveCommand is itself
    /// an Observable. The results of individual invocations can be retrieved via the ExecuteAsync method.
    /// </summary>
    [Obsolete("This type is obsolete and will be removed in a future version of ReactiveUI. Please switch to using ReactiveUI.ReactiveCommand instead.")]
    public class ReactiveCommand<T> : IReactiveCommand<T>, IReactiveCommand
    {
        ...
        public event EventHandler CanExecuteChanged
        {
            add
            {
                if (this.canExecuteDisp == null) this.canExecuteDisp = this.canExecute.Connect();
        ...
        private IConnectableObservable<bool> canExecute;
        private bool canExecuteLatest = false;
        ...
        /// </summary>
        /// <param name="canExecute">The can execute.</param>
        /// <param name="executeAsync">The execute asynchronous.</param>
        ...
        public ReactiveCommand(IObservable<bool> canExecute, Func<object, IObservable<T>> executeAsync, IScheduler scheduler = null)
        {
            ...
            this.canExecute = canExecute.CombineLatest(this.isExecuting.StartWith(false), (ce, ie) => ce && !ie)
        ...
        /// <summary>
        /// Executes a Command and returns the result asynchronously. This method makes it *much*
        /// easier to test ReactiveCommand, as well as create ReactiveCommands who invoke inferior
        /// commands and wait on their results. Note that you **must** Subscribe to the Observable
        /// returned by ExecuteAsync or else nothing will happen (i.e. ExecuteAsync is
        /// lazy) Note also that the command will be executed, irrespective of the current value of
        /// the command's canExecute observable.
        /// </summary>
        ...
        public IObservable<T> ExecuteAsync(object parameter = null)
        {
            var ret = Observable.Create<T>(subj => {
                ...
                var disp = this.executeAsync(parameter)
        ...
        /// <summary>
        /// Executes a Command and returns the result as a Task. This method makes it *much* easier
        /// to test ReactiveCommand, as well as create ReactiveCommands who invoke inferior commands
        /// and wait on their results.
        /// </summary>
        /// <returns>A Task representing a single invocation of the Command.</returns>
        /// <param name="parameter">Don't use this.</param>
        /// <param name="ct">
        /// An optional token that can cancel the operation, if the operation supports it.
        /// </param>
        ...
        /// <summary>
        /// Returns a BehaviorSubject (i.e. an Observable which is guaranteed to return at least one
        /// value immediately) representing the CanExecute state.
        /// </summary>
        public IObservable<bool> CanExecuteObservable
        {
            get
            {
                ...
                if (this.canExecuteDisp != null) return ret;

                return Observable.Create<bool>(subj => {
                    var disp = ret.Subscribe(subj);

                    // NB: We intentionally leak the CanExecute disconnect, it's cleaned up by the
                    // global Dispose. This is kind of a "Lazy Subscription" to CanExecute by the
                    // command itself.
        ...
        {
            get { return this.isExecuting.StartWith(this.inflightCount > 0); }
        }
        ...
        /// <summary>
        /// Executes a Command. Note that the command will be executed, irrespective of the current
        /// value of the command's canExecute observable.
        /// </summary>
        /// <param name="parameter">To be added.</param>
        /// <remarks>To be added.</remarks>
    ...
    /// <summary>
    /// ReactiveCommand Mixins
    /// </summary>
    [Obsolete("This type is obsolete and will be removed in a future version of ReactiveUI. Please switch to using ReactiveUI.ReactiveCommand instead.")]
    public static class ReactiveCommandMixins
    {
        /// <summary>
        /// ToCommand is a convenience method for returning a new ReactiveCommand based on an
        /// existing Observable chain.
        /// </summary>
        /// <param name="This">The this.</param>
        /// <param name="scheduler">The scheduler to publish events on - default is RxApp.MainThreadScheduler.</param>
        /// <returns>A new ReactiveCommand whose CanExecute Observable is the current object.</returns>
        ...
        /// <summary>
        /// A convenience method for subscribing and creating ReactiveCommands in the same call.
        /// Equivalent to Subscribing to the command, except there's no way to release your
        /// Subscription but that's probably fine.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="This">The this.</param>
