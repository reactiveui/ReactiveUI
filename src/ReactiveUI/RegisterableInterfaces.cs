...
namespace ReactiveUI
{
    ...
    public enum BindingDirection
    {
        ...
        /// <summary>
        /// The asynchronous one way Binding
        /// </summary>
        AsyncOneWay = 2,
    }

    /// <summary>
    /// IMessageBus represents an object that can act as a "Message Bus", a simple way for ViewModels
    /// and other objects to communicate with each other in a loosely coupled way. /// Specifying
    /// which messages go where is done via a combination of the Type of the message as well as an
    /// additional "Contract" parameter; this is a unique string used to distinguish between messages
    /// of the same Type, and is arbitrarily set by the client.
    /// </summary>
    public interface IMessageBus : IEnableLogger
    {
        ...
        /// </summary>
        /// <remarks>
        /// If a scheduler is already registered for the specified runtime and contract, this will
        /// overrwrite the existing registration.
        /// </remarks>
        /// <typeparam name="T">The type of the message to listen to.</typeparam>
        /// <param name="scheduler">
        /// The scheduler on which to post the notifications for the specified type and contract.
        /// CurrentThreadScheduler by default.
        /// </param>
        /// <param name="contract">
        /// A unique string to distinguish messages with identical types (i.e. "MyCoolViewModel") -
        /// if the message type is only used for one purpose, leave this as null.
        /// </param>
        void RegisterScheduler<T>(IScheduler scheduler, string contract = null);

        /// <summary>
        /// Listen provides an Observable that will fire whenever a Message is provided for this
        /// object via RegisterMessageSource or SendMessage.
        /// </summary>
        /// <typeparam name="T">The type of the message to listen to.</typeparam>
        /// <param name="contract">
        /// A unique string to distinguish messages with identical types (i.e. "MyCoolViewModel") -
        ...
        /// <summary>
        /// ListenIncludeLatest provides an Observable that will fire whenever a Message is provided
        /// for this object via RegisterMessageSource or SendMessage and fire the last provided
        /// Message immediately if applicable, or null.
        /// </summary>
        /// <typeparam name="T">The type of the message to listen to.</typeparam>
        /// <param name="contract">
        /// A unique string to distinguish messages with identical types (i.e. "MyCoolViewModel") -
        /// if the message type is only used for one purpose, leave this as null.
        /// </param>
        /// <returns>An Observable representing the notifications posted to the message bus.</returns>
        ...
        /// <summary>
        /// Registers an Observable representing the stream of messages to send. Another part of the
        /// code can then call Listen to retrieve this Observable.
        /// </summary>
        /// <typeparam name="T">The type of the message to listen to.</typeparam>
        /// <param name="source">
        ...
        /// RegisterMessageSource instead if you will be sending messages in response to other
        /// changes such as property changes or events.
        /// </summary>
    ...
    /// <summary>
    /// ICreatesObservableForProperty represents an object that knows how to create notifications for
    /// a given type of object. Implement this if you are porting RxUI to a new UI toolkit, or
    /// generally want to enable WhenAny for another type of object that can be observed in a unique way.
    /// </summary>
    ...
    /// <summary>
    /// This interface is the extensible implementation of IValueConverters for Bind and OneWayBind.
    /// Implement this to teach Bind and OneWayBind how to convert between types.
    /// </summary>
    ...
    /// <summary>
    /// Implement this as a way to intercept bindings at the time that they are created and execute
    /// an additional action (or to cancel the binding)
    /// </summary>
    ...
    /// <summary>
    /// Use this Interface when you want to mark a control as recieving View Activation when it
    /// doesn't have a backing ViewModel.
    /// </summary>
    ...
    /// <summary>
    /// IScreen represents any object that is hosting its own routing - usually this object is your
    /// AppViewModel or MainWindow object.
    /// </summary>
    ...
    /// <summary>
    /// interface for CreatesCommandBinding
    /// </summary>
    ...
    /// <summary>
    /// Implement this interface to override how ReactiveUI determines when a View is activated or
    /// deactivated. This is usually only used when porting ReactiveUI to a new UI framework
    /// </summary>
    public interface IActivationForViewFetcher
    {
        /// <summary>
        /// Gets the affinity for view.
        /// </summary>
    ...
    */
    ...
    /// <summary>
    /// ISuspensionHost represents a standardized version of the events that the host operating
    /// system publishes. Subscribe to these events in order to handle app suspend / resume.
    /// </summary>
    public interface ISuspensionHost : IReactiveObject
    {
        /// <summary>
        /// Signals when the application is launching new. This can happen when an app has recently
        /// crashed, as well as the first time the app has been launched. Apps should create their
        /// state from scratch.
        /// </summary>
        IObservable<Unit> IsLaunchingNew { get; set; }

        /// <summary>
        /// Signals when the application is resuming from suspended state (i.e. it was previously
        /// running but its process was destroyed).
        /// </summary>
        IObservable<Unit> IsResuming { get; set; }

        /// <summary>
        /// Signals when the application is activated. Note that this may mean that your process was
        /// not actively running before this signal.
        /// </summary>
        ...
        /// </summary>
        /// <value>
        /// Returns an IDisposable that should be disposed once the application finishes persisting
        /// its state
        /// </value>
        IObservable<IDisposable> ShouldPersistState { get; set; }

        /// <summary>
        /// Signals that the saved application state should be deleted, this usually is called after
        /// an app has crashed
        /// </summary>
        IObservable<Unit> ShouldInvalidateState { get; set; }

        /// <summary>
        /// A method that can be used to create a new application state - usually this method just
        /// calls 'new' on an object.
        /// </summary>
        Func<object> CreateNewAppState { get; set; }

        /// <summary>
        /// The current application state - get a typed version of this via GetAppState'T'. The
        /// "application state" is a notion entirely defined via the client application - the
        /// framework places no restrictions on the object other than it can be serialized.
        /// </summary>
        object AppState { get; set; }
    }

    /// <summary>
    /// ISuspensionDriver represents a class that can load/save state to persistent storage. Most
    /// platforms have a basic implementation of this class, but you probably want to write your own.
    /// </summary>
