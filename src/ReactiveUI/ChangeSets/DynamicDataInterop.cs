...
/// </summary>
internal static class DynamicDataInterop
{
    /// <summary>
    /// Adapts a ReactiveUI change-set stream to a DynamicData change-set stream.
    /// </summary>
    /// <typeparam name="T">The collection item type.</typeparam>
    /// <param name="source">The ReactiveUI change-set stream.</param>
    /// <returns>A DynamicData change-set stream.</returns>
    public static IObservable<IChangeSet<T>> ToDynamicDataChangeSet<T>(this IObservable<IReactiveChangeSet<T>> source)
        where T : notnull
    {
        ...
        where T : notnull
    {
        ...
        private sealed class Sink(IObserver<IChangeSet<T>> downstream) : IObserver<IReactiveChangeSet<T>>
        {
            ...
            private static Change<T> ToDynamicDataChange(ReactiveChange<T> change) =>
