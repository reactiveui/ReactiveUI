...
namespace ReactiveUI
{
    ...
    public static class RxApp
    {
        static RxApp()
        {
            ...
            // Initialize this to false as most platforms do not support range notification for INotifyCollectionChanged
#if WP8 || NETFX_CORE
            ...
            DefaultExceptionHandler = Observer.Create<Exception>(ex => {
                ...
                // NB: If you're seeing this, it means that an ObservableAsPropertyHelper or the
                // CanExecute of a ReactiveCommand ended in an OnError. Instead of silently breaking,
                // ReactiveUI will halt here if a debugger is attached.
        ...
        /// <summary>
        /// MainThreadScheduler is the scheduler used to schedule work items that should be run "on
        /// the UI thread". In normal mode, this will be DispatcherScheduler, and in Unit Test mode
        /// this will be Immediate, to simplify writing common unit tests.
        /// </summary>
        public static IScheduler MainThreadScheduler
        {
            ...
            set
            {
                // N.B. The ThreadStatic dance here is for the unit test case - often, each test will
                // override MainThreadScheduler with their own TestScheduler, and if this wasn't
                // ThreadStatic, they would stomp on each other, causing test cases to randomly fail,
        ...
        /// <summary>
        /// TaskpoolScheduler is the scheduler used to schedule work items to run in a background
        /// thread. In both modes, this will run on the TPL Task Pool (or the normal Threadpool on Silverlight).
        /// </summary>
        ...
        /// <summary>
        /// This Observer is signalled whenever an object that has a ThrownExceptions property
        /// doesn't Subscribe to that Observable. Use Observer.Create to set up what will happen -
        /// the default is to crash the application with an error message.
        /// </summary>
        ...
        /// <summary>
        /// This returns / allows you to override the current SuspensionHost, a class which provides
        /// events for process lifetime events, especially on mobile devices.
        /// </summary>
        ...
        /// <summary>
        /// Returns whether your UI framework is brain-dead or not and will blow up if a
        /// INotifyCollectionChanged object returns a ranged Add
        /// </summary>
