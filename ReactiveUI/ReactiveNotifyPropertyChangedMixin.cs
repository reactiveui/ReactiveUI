            ...
            var r = notifier.Select(x =>
                {
                    ...
                    {
                        throw new InvalidCastException(string.Format("Unable to cast from {0} to {1}.", value.GetType(), typeof(TValue)));
                    }

                    return new ObservedChange<TSender, TValue>(source, expression, (TValue) value);
                });
